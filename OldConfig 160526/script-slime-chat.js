...
// Ajoute le pseudo dans le "badge" externe
messageClone.find('.user-badge').append(buildUser(user))
// Le message garde juste le contenu texte/emotes
messageClone.find('.content').append(buildContent(parts))

  // add animation
  messageClone.find('.message').addClass(`animation-enter-${animations.enter}`)
  // add user's badge animatiion
  messageClone.find('.user-badge').addClass(`animation-enter-${animations.enter}`)
  ...
  messageClone.find('.user-badge').css({
  '--userColor': user.color,
  '--userColorLight': slime2.color.light(user.color),
  '--userColorDark': slime2.color.dark(user.color),
})
